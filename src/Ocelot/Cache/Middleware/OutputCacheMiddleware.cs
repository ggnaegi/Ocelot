...
public class OutputCacheMiddleware : OcelotMiddleware
{
    ...
    public OutputCacheMiddleware(RequestDelegate next,
        ...
        IOcelotCacheProvider cacheProvider)
    ...
    public async Task Invoke(HttpContext httpContext)
    {
        ...
        if (!downstreamRoute.IsCached || _cacheProvider == null)
        {
        ...
        var downStreamRequestCacheKey = await _cacheProvider.GetCacheKeyGenerator()
            .GenerateRequestCacheKey(downstreamRequest, downstreamRoute);

        Logger.LogDebug($"Started checking cache for the '{downstreamUrlKey}' key.");

        var cached = _cacheProvider.GetResponseCache()
            .Get(downStreamRequestCacheKey, downstreamRoute.CacheOptions.Region);
        ...
        _cacheProvider.GetResponseCache().Add(downStreamRequestCacheKey, cached,
            TimeSpan.FromSeconds(downstreamRoute.CacheOptions.TtlSeconds), downstreamRoute.CacheOptions.Region);
